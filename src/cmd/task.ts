...
import { createApprovalHandler } from '../lib/approvals.js';
import {
  CLAUDE_EFFORTS,
  CLAUDE_MODELS,
...
export async function commandTask(argv: string[]): Promise<void> {
  ...
  if (background) {
    ...
    // Human output: a short line plus the commands to manage the task.
  ...
  try {
    const current = readJob(cwd, jobId) ?? job;
    ...
    if (options.json) {
      ...
      process.stderr.write(
        `[coder] task=${jobId} status=${result.status === 0 ? 'completed' : 'failed'}\n`,
      );
    }
